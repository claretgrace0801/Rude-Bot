...
languages = {"english":"en-us", "hindi":"hi"}
lang_code = "en-us"
...
async def text_to_speech(message, content):
    ...
    tts_var = gTTS(content, lang=lang_code, lang_check=False)
...
@bot.command(name='lang', help="change the default language \nLanguages:\nenglish\nhindi")
async def language_select(ctx, arg):
    global lang_code
    if arg in languages.keys():
        lang_code = languages[arg]
        await ctx.send(f"Language changed to {arg}")
    else:
        await ctx.send(f"{arg} is not an available option. Current language is {lang}")
